...
class Decoder(nn.Module):
    ...
    def forward(self, y_1, y_2, y_3) -> List[torch.Tensor]:
        x_3 = y_3
        ...
        out_1, feat_3 = self.decoder_1(x_1)
...
class Decoder_Level(nn.Module):
    ...
    def forward(self, x):
        ...
        feature = F.interpolate(x, scale_factor=2.0, mode="bilinear", align_corners=True, recompute_scale_factor=True)
        return out, feature
    ...
    def __init__(self, in_channel, d_list, inter_num):
        ...
        for i in range(len(d_list)):
            dense_conv = conv_relu(
                in_channel=c, out_channel=inter_num, kernel_size=3, dilation_rate=d_list[i], padding=d_list[i]
            )
...
class SAM(nn.Module):
    ...
    def forward(self, x):
        x_0 = x
        x_2 = F.interpolate(x, scale_factor=0.5, mode="bilinear", align_corners=True, recompute_scale_factor=True)
        x_4 = F.interpolate(x, scale_factor=0.25, mode="bilinear", align_corners=True, recompute_scale_factor=True)

        y_0 = self.basic_block(x_0)
        y_2 = self.basic_block_2(x_2)
        y_4 = self.basic_block_4(x_4)

        y_2 = F.interpolate(y_2, scale_factor=2.0, mode="bilinear", align_corners=True, recompute_scale_factor=True)
        y_4 = F.interpolate(y_4, scale_factor=4.0, mode="bilinear", align_corners=True, recompute_scale_factor=True)
...
class ESDNet(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = Decoder(
            en_num=en_feature_num, feature_num=de_feature_num, inter_num=de_inter_num, sam_number=sam_number
        )
